Rem
Rem $Header: dmglcdem.sql 08-may-2007.15:12:07 bmilenov Exp $
Rem
Rem dmglcdem.sql
Rem
...
SELECT setting_name, setting_value
  FROM TABLE(DBMS_DATA_MINING.GET_MODEL_SETTINGS('GLMC_SH_Clas_sample'))
...
SELECT attribute_name, attribute_type
  FROM TABLE(DBMS_DATA_MINING.GET_MODEL_SIGNATURE('GLMC_SH_Clas_sample'))
...
--
-- Global statistics
SELECT * FROM TABLE(dbms_data_mining.get_model_details_global('GLMC_SH_Clas_sample'))
...
SELECT * FROM   
  (SELECT * FROM TABLE(dbms_data_mining.get_model_details_glm('GLMC_SH_Clas_sample')) 
  ORDER BY class, attribute_name, attribute_value)
WHERE ROWNUM < 11;  
    
-- Row diagnostics
SELECT * FROM 
  (SELECT * FROM glmc_sh_sample_diag 
   ORDER BY case_id)
WHERE ROWNUM < 11;
