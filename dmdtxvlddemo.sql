Rem
Rem $Header: dmdtxvlddemo.sql 11-jul-2005.13:21:49 ktaylor Exp $
Rem
...
BEGIN       
 ...
 -- The type of the actual and predicted should be the same as the 
 -- target column, in this case it happens to be NUMBER
 EXECUTE IMMEDIATE 'create table dmdtxvld_c (iter number,'||
                   'actual number(10),predicted '||
                   'number(10), count number)';
 ...
 BEGIN
  execute immediate 'drop table '|| tabname_p;
...
-- Open tgtcur using tgtname,tabname;
 ...
 sqltxt := sqltxt || ' select t1.*,mod(rownum,' ||nfolds ||') pn from '||
           ' (select t.*, ORA_HASH(rownum) randrow from '|| tabname ||
           ' t where '|| tgtname || ' = '''||tgtval||''' order by randrow) t1';
 LOOP 
   fetch tgtcur into tgtval;
   exit when tgtcur%notfound;

   sqltxt := sqltxt || 
           ' union all select t1.*,mod(rownum,' ||nfolds ||') pn from ' ||
           ' (select t.*, ORA_HASH(rownum) randrow from '|| tabname ||
           ' t where '|| tgtname || ' = '''||tgtval||''' order by randrow) t1';
 END LOOP;
 close tgtcur;
 ...
 LOOP
  ...
  execute immediate 'create or replace view xvld_tmp_bld_v as select * from ' ||
                    tabname_p || ' WHERE pn != '|| iter;

  -- We will test using rows in current partition
  execute immediate 'create or replace view xvld_tmp_tst_v as select * from ' ||
            tabname_p || ' WHERE pn = '|| iter;
  ...
  BEGIN
    ...
    execute immediate 
            ...
            'PREDICTION('||
             model_name ||' USING *) AS predicted_target_value,'||
            'COUNT(*) AS value '||
            'FROM xvld_tmp_tst_v '||
            'GROUP BY '||tgtname||', PREDICTION('||
                                     model_name|| ' USING *) '||
            'ORDER BY 1,2';
...
exec DBMS_OUTPUT.PUT_LINE('Model accuracy by iteration');
select a.iter, avg(round(correct/total,4)) as accuracy from (select iter,sum(count) as correct from dmdtxvld_c where actual=predicted group by iter) a,(select iter,sum(count) as total from dmdtxvld_c group by iter) b where a.iter=b.iter group by rollup (a.iter);

-- Show confusion matrix by iteration and rolled up across iterations
exec DBMS_OUTPUT.PUT_LINE('Confusion Matrix by iteration');
select * from (select iter, actual,predicted, sum(count) count from dmdtxvld_c group by rollup (actual,predicted,iter)) where predicted is not null order by iter,actual,predicted;
