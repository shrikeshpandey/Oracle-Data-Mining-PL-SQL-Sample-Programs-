Rem
Rem $Header: dmabdemo.sql 19-jan-2005.18:31:13 jcjeon Exp $
Rem
Rem dmabdemo.sql
Rem
...
Rem
Rem    NAME
Rem      dmabdemo.sql - Sample program for DBMS_DATA_MINING package.
Rem
Rem    DESCRIPTION
Rem      This script creates a classification model
Rem      using the ABN algorithm and data in the
Rem      SH (Sales History) schema in the RDBMS.
Rem
...
Rem    ktaylor     07/11/05 - minor editing of comments 
Rem    jcjeon      01/17/05 - add column format 
Rem    ramkrish    12/20/04 - add ORDER BY in missing values cursor 
Rem    jyarmus     11/01/04 - modify binning, ADD settings examples AS comments
Rem    ramkrish    10/01/04 - add data analysis and comments/cleanup
...
SELECT setting_name, setting_value
  FROM TABLE(DBMS_DATA_MINING.GET_MODEL_SETTINGS('ABN_SH_Clas_sample'))
...
SELECT attribute_name, attribute_type
  FROM TABLE(DBMS_DATA_MINING.GET_MODEL_SIGNATURE('ABN_SH_Clas_sample'))
