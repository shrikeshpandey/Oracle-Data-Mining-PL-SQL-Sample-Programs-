Rem
Rem $Header: dmglrdem.sql 08-may-2007.15:12:33 bmilenov Exp $
Rem
Rem dmglrdem.sql
Rem
...
SELECT setting_name, setting_value
  FROM TABLE(DBMS_DATA_MINING.GET_MODEL_SETTINGS('GLMR_SH_Regr_sample'))
...
SELECT attribute_name, attribute_type
  FROM TABLE(DBMS_DATA_MINING.GET_MODEL_SIGNATURE('GLMR_SH_Regr_sample'))
...
--
-- Global statistics
SELECT * FROM TABLE(dbms_data_mining.get_model_details_global('GLMR_SH_Regr_sample'))
...
SELECT * FROM   
  (SELECT * FROM TABLE(dbms_data_mining.get_model_details_glm('GLMR_SH_Regr_sample')) 
  ORDER BY class, attribute_name, attribute_value)
WHERE ROWNUM < 11;  
    
-- Row diagnostics
SELECT * FROM 
  (SELECT * FROM glmr_sh_sample_diag 
   ORDER BY case_id)
WHERE ROWNUM < 11;
...
CREATE VIEW mining_data_build_v2 AS (SELECT CUST_id, CUST_gender, age,
  CUST_MARITAL_status, COUNTRY_name, education,
  occupation, HOUSEHOLD_size, YRS_residence, AFFINITY_card, BULK_PACK_diskettes,
  FLAT_PANEL_monitor, HOME_THEATER_package, Y_BOX_games,
  OS_DOC_SET_kanji FROM mining_data_build_v);
